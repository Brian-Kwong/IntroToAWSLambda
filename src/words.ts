const words : string[]= ["aback", "abase", "abate", "abbey", "abbot", "abhor", "abide", "abled", 
...
// Picks a random word from the list based on Today
export function pickRandomWord() : string{
    // Applies a hashing function for the current day to get a word
    const now = new Date(Date.now()); // Uses UTC time
...
// Checks the word against the word of the day
export function checkWord(word : string){
    const todayWord = pickRandomWord();
    if (words.includes(word) === false) { // Is it a valid word?
        return "Not a word";
    }
